...
impl MessageHandler for AllocateMessageHandler {
    async fn handle(&self, base_session_id: String, data: &[u8], message_id: Option<String>) -> Result<String, ApiError> {
        ...
        tracing::info!("session {:?} allocate resource {:?}", base_session_id, resources);
        if let Ok((success_resources, failed_resources)) = self.resource_app.handle_allocate(&base_session_id, resources).await {
            tracing::info!("session {:?} allocate resource, success_resources: {:?}, failed_resources: {:?}", base_session_id, success_resources, failed_resources);
            // 构建响应
